...
def logit_logl(
    ...
    if label_observation_parameters is not None:
        ...
        value += np.sum(np.log(pos_part)) + np.sum(np.log(neg_part))
    else:
        value -= np.sum(np.logaddexp(0, -alpha - scores_pos))
        value -= np.sum(np.logaddexp(0, alpha + scores_neg))
...
def logit_derivative_alpha(scores_pos, scores_neg, alpha, label_observation_probs=None):
    scores_pos = np.asarray(scores_pos, dtype=float)
    scores_neg = np.asarray(scores_neg, dtype=float)
    ...
    return pos_factor * np.sum(1 / pos_denominator) + neg_factor * np.sum(1 / neg_denominator)
...
def logit_derivative_label_probs(
    ...
    return np.array(
        [
            d_00_prior + np.sum(d_00_neg) + np.sum(d_00_pos),
            d_11_prior + np.sum(d_11_neg) + np.sum(d_11_pos),
        ]
    )


def logit_subderivative_theta(alignments, labels, alpha, label_observation_probs=None):
        ...
        for aln_crd, mir_crd in enumerate(crd_miR):
            if mir_crd != -1:
                ...
                if nt2 == "-":
                    ...
                    M_subd[NUCL_DICT[nt1], NUCL_DICT[nt2], mir_crd] += scaling_factor
            else:
                mir_gap_count += 1
